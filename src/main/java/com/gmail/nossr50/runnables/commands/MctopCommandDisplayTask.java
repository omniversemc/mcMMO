    ...
    private void displayChat() {
        ...
        for (PlayerStat stat : userStats) {
            ...
            if (sender instanceof Player) {
                // &9&l| &b%s &8• &f%s &8- &b%s
                // | %2 - %s - %s
                sender.sendMessage(String.format("§9§l| §b%s §8• §f%s §8- §b%s", place, stat.name, stat.statVal));
            } else {
                sender.sendMessage(String.format("%s%2d. %s - %s", place, stat.name, stat.statVal));
            }
